...
(function() {
  ...
  BackgroundCommands = {
    ...
    nextFrame: function(count, frameId) {
      return chrome.tabs.getSelected(null, function(tab) {
        ...
        frames = frameIdsForTab[tab.id] = slice.call(frames.slice(count)).concat(slice.call(frames.slice(0, count)));
        return chrome.tabs.sendMessage(tab.id, {
          name: "focusFrame",
          frameId: frames[0],
          highlight: true
  ...
  setBadge = (function() {
    var current, timer, updateBadge;
    current = null;
    timer = null;
    updateBadge = function(badge, tabId) {
      return function() {
    ...
    return function(request, sender) {
      var badge;
      badge = request.badge;
      if ((badge != null) && badge !== current) {
        current = badge;
        if (timer) {
          clearTimeout(timer);
        }
        return timer = setTimeout(updateBadge(badge, sender.tab.id), 50);
      }
    };
  })();
  ...
  handleFrameFocused = function(request, sender) {
    ...
    if (frameIdsForTab[tabId] != null) {
      return frameIdsForTab[tabId] = [request.frameId].concat(slice.call(frameIdsForTab[tabId].filter(function(id) {
          return id !== request.frameId;
        })));
    }
  };
